using System;
using System.ComponentModel;

namespace CONFDB.Entities
{
	/// <summary>
	///		The data structure representation of the 'AccessType' table via interface.
	/// </summary>
	/// <remarks>
	/// 	This struct is generated by a tool and should never be modified.
	/// </remarks>
	public interface IAccessType 
	{
		/// <summary>			
		/// ID : 
		/// </summary>
		/// <remarks>Member of the primary key of the underlying table "AccessType"</remarks>
		System.Int32 Id { get; set; }
				
		
		
		/// <summary>
		/// Name : 
		/// </summary>
		System.String  Name  { get; set; }
		
		/// <summary>
		/// DisplayName : 
		/// </summary>
		System.String  DisplayName  { get; set; }
		
		/// <summary>
		/// Description : 
		/// </summary>
		System.String  Description  { get; set; }
		
		/// <summary>
		/// Value : 
		/// </summary>
		System.Int32  Value  { get; set; }
		
		/// <summary>
		/// RetailLDApplicable : 
		/// </summary>
		System.Boolean  RetailLdApplicable  { get; set; }
		
		/// <summary>
		/// WholesaleLDApplicable : 
		/// </summary>
		System.Boolean  WholesaleLdApplicable  { get; set; }
		
		/// <summary>
		/// Billable : 
		/// </summary>
		System.Boolean  Billable  { get; set; }
		
		/// <summary>
		/// Enabled : 
		/// </summary>
		System.Boolean  Enabled  { get; set; }
			
		/// <summary>
		/// Creates a new object that is a copy of the current instance.
		/// </summary>
		/// <returns>A new object that is a copy of this instance.</returns>
		System.Object Clone();
		
		#region Data Properties


		/// <summary>
		///	Holds a collection of entity objects
		///	which are related to this object through the relation _dnisAccessTypeId
		/// </summary>	
		TList<Dnis> DnisCollection {  get;  set;}	


		/// <summary>
		///	Holds a collection of entity objects
		///	which are related to this object through the relation _accessType_ProductRateAccessTypeId
		/// </summary>	
		TList<AccessType_ProductRate> AccessType_ProductRateCollection {  get;  set;}	


		/// <summary>
		///	Holds a collection of entity objects
		///	which are related to this object through the relation _ratedCdrAccessTypeId
		/// </summary>	
		TList<RatedCdr> RatedCdrCollection {  get;  set;}	

		#endregion Data Properties

	}
}


